...
class UserController extends ControllerMVC {
  ...
  TextEditingController referalCodeUsedController = TextEditingController();
...
// }
    ...
    try {
      String response = await repository.userLogin(
          accountnumber.toUpperCase(), passwordController.text);
      ...
      if (jsonResponse['success'] == true) {
        ...
        String rr = await repository.updatetokken(jsonResponse['user']['_id'],
            devicetokken, jsonResponse['accessToken']);
        log(rr);
        Navigator.of(context).pushNamed('/NavBar',
            arguments: RouteArgument(currentTab: 0, heroTag: "0"));
      } else {
        showDialog(
            context: context,
            builder: (context) {
  ...
  register(String token) async {
    ...
    try {
      log(assignednumber);
      String response = await repository.userRegister(
          assignednumber.toUpperCase(),
    ...
    try {
      ...
      if (jsonResponse['success'] == true) {
        ...
        Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => RetrieveCodeWidget(emailController.text.trim(),
                jsonResponse['code'], jsonResponse['user_id'])));
      } else {
        showDialog(
            context: context,
            builder: (context) {
  ...
  sendRegisterVerification() async {
    try {
      String response =
          await repository.sendRegisterEmail(emailController.text.trim());
      final jsonResponse = json.decode(response);
      if (jsonResponse['success'] == true) {
        print('code------------' + jsonResponse['code'].toString());
        setState(() {
        ...
        Fluttertoast.showToast(
            msg: 'The email containing the verification code has been sent.');
      } else {
        showDialog(
            context: context,
            builder: (context) {
